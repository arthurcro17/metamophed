...
def update_products(request):
    ...
    try:
        ...
        for full_product in products:
            ...
            ai_data_response = get_AI_product_data(request, model, data=data)
...
def get_AI_product_data(request, model="gpt-3.5-turbo", data=None):
    try:
        ...
        system_prompt = """
                        You are an experienced E-Commerce copywriter. Provide improved product data using the given data. 
                        Generate "new" data from "old" data, provided "prompt", and "additional_product_info". 
                        Return proper JSON formatted object with one less key than input, removing "additional_product_info" key. 
...
def check_premium(request):
    ...
    if store is None:
        return JsonResponse({"response":"Not logged in!"})
    return JsonResponse({"premium": store.premium})